        ...
        static char[][] Rotate(List<string> entries, int times) // times equals rotationAngle / 90
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                entries[i] =
                    ...
                    new string(' ',
                        cols - entries[i]
                            .Length); // increase the length of all strings, so it is equal to the longest (you can use LINQ if you want)
            }
            ...
            else // the rotation is 90 or 270 degrees
            {
                ...
                for (int i = 0; i < cols; i++)
                {
                    ...
                    {
                        result[i][j] =
                            entries[times == 3 ? j : rows - 1 - j]
                                [times == 3 ? cols - 1 - i : i]; // ternary operator used to determine the correct index
                    }
                }
